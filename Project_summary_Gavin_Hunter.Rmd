...
knitr::kable(head(seaspurge, n = 10), format = "html", caption = "Basic population diversity indices for sea spurge populations collected across five Australian States") %>% 
  ...
  footnote(number = c("N: Number of samples", "Ho: Observed Heterozygosity",
                      "Hs: Observed Gene diversity", "Ar: Allelic Richness",
                      "Fis: Inbreeding coefficient", "Pa: Number of Private alleles"))
```


```{r, out.width='100%', fig.align='center', fig.height= 12, fig.width=15, fig.cap="Figs.1-4: Plots of Observed heterozygosities, Gene diversity, private alleles and inbreeding coefficients for populations of sea spurge based on biallelic SNP data."}
...
plot1 <- seaspurge %>% 
  ...
  labs(
    title = "Figure 1: Gene Diversity vs. Heterozygosity",
...
plot2 <- seaspurge %>%
  ...
  labs(
    title = "Figure 2: Observed heterozygosity for each sea spurge population",
...
plot3 <- seaspurge %>% 
  ...
  geom_col() +
  labs(
    title = "Figure 3: Total number of private alleles across all populations within a State",
    x = "State",
...
![](https://raw.githubusercontent.com/tidyverse/ggplot2/master/man/figures/logo.png){width=100px}
...
population genetics and genomics to apply them even more to my typical analysis workflows.
...
research and open other data analysis pathways.
